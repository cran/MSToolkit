createDropout <- function(
  ...
  idCol = "SUBJ",                     #@ name of the subject varuable in the data  
  timeCol = "TIME",                 #@ name of the time column
  flagName = "MISSING",             #@ name of the missing variable to create
  ...                               #@ to pass to dropFunc
){
  ###############################################################################
  ...
  if( !missing(timeCol)){
    ...
    timeCol %in% names(data) || ectdStop("The `$timeCol` column is not in the supplied data")
  }
  idCol %in% names(data) || ectdStop("The `$idCol` column is not in the suppplied dataset") 
  ...
  missingFlag %of% "try-error" &&  ectdStop("Error occuring when calling the dropout function: $missingFlag" ) 
